...
class FFmpegFrame {
    ...
    ///
    lazy var avFrame: AVFrame = pointer.pointee
...
//    var firstSampleIndex: Int32
    ...
    ///
    init?(readingFrom codecCtx: UnsafeMutablePointer<AVCodecContext>) {
...
//        self.firstSampleIndex = 0
    ...
    init(readingFromFIFO fifo: FFmpegAudioFIFO, encoderCtx: AVCodecContext, frameSize: Int32) {
        
        // Allocate memory for the frame.
        self.pointer = av_frame_alloc()
        
//        self.sampleFormat = FFmpegSampleFormat(encapsulating: encoderCtx.sample_fmt)
//        self.firstSampleIndex = 0
        
        avFrame.nb_samples = frameSize
        avFrame.channel_layout = encoderCtx.channel_layout
        avFrame.format = encoderCtx.sample_fmt.rawValue
        avFrame.sample_rate = encoderCtx.sample_rate
        
        av_frame_get_buffer(pointer, 0)
        
        var rawPtr = UnsafeMutableRawPointer(mutating: avFrame.extended_data)
        av_audio_fifo_read(fifo.pointer, &rawPtr, frameSize)
    }
